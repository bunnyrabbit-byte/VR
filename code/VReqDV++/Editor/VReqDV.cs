...
public class MainMenu : EditorWindow
{
    ...
    private void DisplayBehaviorForm(List<BehaviorRule> behaviors, int version)
    {
        ...
        foreach (var rule in behaviors)
        {
            ...
            string actorsStr = rule.Actors != null ? string.Join(", ", rule.Actors) : "None";
            EditorGUILayout.LabelField("Actors: ", actorsStr);
    ...
    private void DisplayConditionNode(ConditionNode node, int indentLevel = 0)
    {
        ...
        if (!string.IsNullOrEmpty(node.XRinteraction))
        {
             EditorGUILayout.LabelField($"{indent}XR Interaction: {node.XRinteraction}");
        }
